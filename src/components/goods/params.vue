...
export default {
  ...
  methods: {
    ...
    editParams() {
      this.$refs.editFormRef.validate(async (valid) => {
        ...
        const { data: res } = await this.$http.put(`categories/${this.cateId}/attributes/${this.editForm.attr_id}`, {
          ...
          attr_sel: this.activeName,
          attr_vals: this.editForm.attr_vals //!这里很重要，解决了后来遇到的一个BUG 编辑参数完参数以后，这里新的参数，下面不带以前attr_vals 这个值 所以需要在这里强制的穿这个参数
        })
        if (res.meta.status !== 200) {
    ...
    showInput(row) {
      ...
      // 让文本框自动获得焦点
      // !$nextTick: 当页面上的元素被重新渲染之后，才会执行回调函数中的代码，这个很重要，当视图更新以后想要拿到页面的数据，需要通过$nextTick(()=>{}) 把获取页面数据的方法放到这个箭头函数里面，就可以了啊
      this.$nextTick((_) => {
